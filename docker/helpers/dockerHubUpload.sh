...
# This script used in Travis CI to Build Production Image using Docker and Upload it to Docker Hub with appopriate tags.
...
then
  docker tag hb-frontend-image-prod sandunwebdev/hb-frontend-image-prod:master;

  docker push sandunwebdev/hb-frontend-image-prod:master;
elif [ "$TRAVIS_BRANCH" == "release" ] && [ "$TRAVIS_PULL_REQUEST" == "false" ]
then
  docker tag hb-frontend-image-prod sandunwebdev/hb-frontend-image-prod:release;
  docker tag hb-frontend-image-prod sandunwebdev/hb-frontend-image-prod:latest;

  docker push sandunwebdev/hb-frontend-image-prod:release;
  docker push sandunwebdev/hb-frontend-image-prod:latest;
fi
